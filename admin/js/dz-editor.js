...
(function () {
  ...
  var ICONS = {
    ...
    video: '<svg viewBox="0 0 24 24"><path d="M17 10.5V7c0-.55-.45-1-1-1H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.55 0 1-.45 1-1v-3.5l4 4v-11l-4 4zM14 13h-3v3H9v-3H6v-2h3V8h2v3h3v2z"/></svg>',
  ...
  DzEditor.prototype._build = function () {
    ...
    g5.appendChild(self._btn('video', '插入视频', ICONS.video, function () { self._openVideoModal(); }));
  ...
  DzEditor.prototype._openVideoModal = function () {
    var self = this;
    saveSelection();
    var mask = el('div', { class: 'dz-modal-mask' });
    var modal = el('div', { class: 'dz-modal' });
    var header = el('div', { class: 'dz-modal-header', html: '<span>插入视频</span>' });
    var closeBtn = el('button', { class: 'dz-modal-close', text: '×' });
    header.appendChild(closeBtn);
    var body = el('div', { class: 'dz-modal-body' });
    ...
    // 选项卡
    var tabs = el('div', { class: 'dz-modal-tabs' });
    var tabUrl = el('div', { class: 'dz-modal-tab active', text: '视频链接' });
    var tabEmbed = el('div', { class: 'dz-modal-tab', text: '嵌入代码' });
    tabs.appendChild(tabUrl);
    tabs.appendChild(tabEmbed);

    // URL 面板
    var urlPanel = el('div');
    var urlInput = el('input', { type: 'url', placeholder: '支持视频链接或平台链接（YouTube、B站、抖音等）' });
    var tipEl = el('div', { class: 'dz-modal-tip', html: '支持格式：<br>• 直接视频链接（.mp4, .webm, .ogg）<br>• YouTube: https://www.youtube.com/watch?v=...<br>• B站: https://www.bilibili.com/video/BV...<br>• 抖音: https://www.douyin.com/video/...' });
    urlPanel.appendChild(el('div', { class: 'dz-modal-field' }, [el('label', { text: '视频链接' }), urlInput]));
    urlPanel.appendChild(tipEl);

    // 嵌入代码面板
    var embedPanel = el('div', { style: 'display:none;' });
    var embedInput = el('textarea', { placeholder: '粘贴视频嵌入代码，例如：<iframe src="..."></iframe>', rows: 6 });
    embedPanel.appendChild(el('div', { class: 'dz-modal-field' }, [el('label', { text: '嵌入代码' }), embedInput]));

    body.appendChild(tabs);
    body.appendChild(urlPanel);
    body.appendChild(embedPanel);
    ...
    tabUrl.addEventListener('click', function () {
      tabUrl.classList.add('active'); tabEmbed.classList.remove('active');
      urlPanel.style.display = ''; embedPanel.style.display = 'none';
      okBtn.textContent = '插入'; urlInput.focus();
    });
    tabEmbed.addEventListener('click', function () {
      tabEmbed.classList.add('active'); tabUrl.classList.remove('active');
      embedPanel.style.display = ''; urlPanel.style.display = 'none';
    ...
    cancelBtn.addEventListener('click', close);
    okBtn.addEventListener('click', function () {
      if (tabEmbed.classList.contains('active')) {
        var embedCode = embedInput.value.trim();
        if (!embedCode) { embedInput.focus(); return; }
        insertVideo('<div class="video-embed">' + embedCode + '</div>');
        return;
      }
      ...
      var videoHtml = self._parseVideoUrl(url);
      if (videoHtml) {
        insertVideo(videoHtml);
      } else {
        alert('无法识别的视频链接，请检查链接格式或使用嵌入代码方式');
      }
    });
    mask.addEventListener('click', function (e) { if (e.target === mask) close(); });
  };
  ...
  DzEditor.prototype._parseVideoUrl = function (url) {
    // 直接视频文件链接
    if (/\.(mp4|webm|ogg)(\?|$)/i.test(url)) {
      return '<div class="video-container"><video controls src="' + escapeAttr(url) + '" style="max-width:100%;height:auto;"></video></div>';
    }

    // YouTube
    var ytMatch = url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/);
    if (ytMatch) {
      return '<div class="video-container"><iframe src="https://www.youtube.com/embed/' + ytMatch[1] + '" frameborder="0" allowfullscreen style="width:100%;height:400px;max-width:100%;"></iframe></div>';
    }

    // Bilibili - 支持多种格式
    var biliMatch = url.match(/(?:bilibili\.com\/video\/|b23\.tv\/)(BV[a-zA-Z0-9]+)/);
    if (biliMatch) {
      return '<div class="video-container"><iframe src="https://player.bilibili.com/player.html?bvid=' + biliMatch[1] + '&high_quality=1&danmaku=0" frameborder="0" allowfullscreen style="width:100%;height:400px;max-width:100%;"></iframe></div>';
    }

    // 抖音 - 支持完整链接和短链接
    var douyinMatch = url.match(/douyin\.com\/video\/(\d+)/) || url.match(/v\.douyin\.com\/([a-zA-Z0-9]+)/);
    if (douyinMatch) {
      // 如果是短链接，提示用户
      if (douyinMatch[0].indexOf('v.douyin.com') !== -1) {
        return '<div class="video-container"><div style="padding:20px;background:#f5f5f5;border-radius:4px;text-align:center;color:#666;">抖音短链接需要手动获取视频ID<br>请复制完整视频链接：https://www.douyin.com/video/视频ID</div></div>';
      }
      return '<div class="video-container"><iframe src="https://www.douyin.com/iframe/video/' + douyinMatch[1] + '" frameborder="0" allowfullscreen style="width:100%;height:600px;max-width:100%;"></iframe></div>';
    }

    // 腾讯视频
    var qqMatch = url.match(/v\.qq\.com\/.*\/([a-zA-Z0-9]+)\.html/);
    if (qqMatch) {
      return '<div class="video-container"><iframe src="https://v.qq.com/txp/iframe/player.html?vid=' + qqMatch[1] + '" frameborder="0" allowfullscreen style="width:100%;height:400px;max-width:100%;"></iframe></div>';
    }
